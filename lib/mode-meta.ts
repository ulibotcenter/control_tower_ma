...
> = {
  operate: {
    ...
    homeSub:
      "Cada operação segue os seis pilares: Preparação, Due Diligence, Estruturação, Aprovações, Fechamento e Integração. O semáforo no topo explica o ritmo. Ainda não há LOI nem SPA. Closing-alvo da Loopert: jan/2027, flexível.",
...
export function viewChrome(mode: MeetingMode, present: boolean) {
  return {
    ...
    showIa: mode === "operate" && !present,
    showRh: mode !== "target" && !present,
    showInternalReading: mode === "operate" && !present,
    showThesis: mode === "operate" && !present,
    showCap: mode === "operate" && !present,
    showPeople: mode === "operate" && !present,
    showNotes: !present,
    showDocs: !present,
