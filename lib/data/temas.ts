 ...
 */
import { isPendingInboxDoc, isScanFileOverlay, isScanFolderDoc } from "./doc-groups";
...
function coletar(bundle: DealBundle, tema: TemaSlug): TemaHit[] {
  ...
  const docs = bundle.documents
    .filter((d) => !isScanFolderDoc(d) && !isScanFileOverlay(d) && !isPendingInboxDoc(d) && caiNoTema(d, tema))
    .map((d) => hit(d, "doc"));
